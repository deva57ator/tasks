...
function computeAggregatedWorkdayStats(now=Date.now(),options){const base=getManualWorkdayStats();let delta={timeMs:0,doneCount:0};let includeDelta=!workdayState||workdayState.closedManually!==true;if(workdayState&&workdayState.closedManually===true){const endTs=typeof workdayState.end==='number'&&isFinite(workdayState.end)?workdayState.end:null;if(endTs===null||now<=endTs){includeDelta=true}}
if(includeDelta){delta=computeWorkdayProgress(now,options)}return{timeMs:base.timeMs+delta.timeMs,doneCount:base.doneCount+delta.doneCount,base,delta}}
