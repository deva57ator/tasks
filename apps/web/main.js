...
function renderYearPlanActivities(monthMeta,items){if(!Array.isArray(monthMeta))return;if(!Array.isArray(items))return;for(const meta of monthMeta){if(!meta||!meta.layer){continue}meta.layer.innerHTML=''}for(const meta of monthMeta){if(!meta||!meta.layer)continue;const monthItems=items.filter(entry=>entry.month-1===meta.index);if(!monthItems.length)continue;const segments=getYearPlanSegmentsForMonth(monthItems,meta.daysInMonth);for(const segment of segments){const start=Math.max(1,segment.startDay);const end=Math.max(start,segment.endDay);const block=document.createElement('div');block.className='year-activity';const leftPercent=segment.colCount?segment.slotIndex/segment.colCount*100:0;const widthPercent=segment.colCount?100/segment.colCount:100;const leftOffset=YEAR_PLAN_COLUMN_GAP/2;const widthOffset=YEAR_PLAN_COLUMN_GAP;block.style.top=`${(start-1)*YEAR_PLAN_DAY_HEIGHT}px`;block.style.height=`${(end-start+1)*YEAR_PLAN_DAY_HEIGHT}px`;block.style.left=YEAR_PLAN_COLUMN_GAP?`calc(${leftPercent}% + ${leftOffset}px)`:leftPercent+'%';block.style.width=YEAR_PLAN_COLUMN_GAP?`calc(${widthPercent}% - ${widthOffset}px)`:widthPercent+'%';if(segment.isFirst){const title=document.createElement('div');title.className='year-activity-title';title.textContent=segment.item.title||'Без названия';const duration=document.createElement('div');duration.className='year-activity-duration';duration.textContent=`${segment.item.endDay-segment.item.startDay+1} дней`;block.append(title,duration)}meta.layer.appendChild(block)}}}
