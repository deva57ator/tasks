import {
  ...
  ThemeStore,
  WorkdayStore,
  registerTaskWriteHandler,
  registerProjectWriteHandler,
  registerArchiveWriteHandler,
  registerWorkdayWriteHandler,
  getStorageMode,
  setStorageMode as applyStorageMode,
  getApiSettings as readApiSettings,
  updateApiSettings as persistApiSettings,
  onStorageModeChange,
  onApiSettingsChange
} from '../state.js';
...
if (storageMode !== STORAGE_MODES.SERVER) {
  ...
  applyStorageMode(storageMode);
}

let apiSettings = { baseUrl: DEFAULT_API_BASE, ...readApiSettings() };
...
if (typeof rawBase !== 'string' || !rawBase.trim()) {
  ...
  apiSettings.baseUrl = trimmedBase;
  if (trimmedBase !== rawBase) {
    persistApiSettings(apiSettings);
  }
}
...
registerTaskWriteHandler(()=>{afterTasksPersisted()});
registerProjectWriteHandler(handleServerProjectsWrite);
registerArchiveWriteHandler(handleServerArchiveWrite);
registerWorkdayWriteHandler(handleServerWorkdayWrite);
