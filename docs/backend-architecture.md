...
### `DB_PATH` и рабочая директория процесса

Это canonical место, где стоит держать в голове поведение `DB_PATH`.

Что важно:

- backend резолвит `DB_PATH` относительно `process.cwd()`;
- один и тот же относительный путь может указывать на разные SQLite-файлы в зависимости от того, откуда запущен процесс;
- workflow и systemd могут подставлять `DB_PATH` явно, и это важнее локального default.

Проверяйте это всегда, если:

- миграция “успешна”, но данные не поменялись;
- staging и production видят разные наборы данных;
- локально сервис стартует, но пишет не в ту базу;
- после деплоя health-check зелёный, но бизнес-данные выглядят старыми.

Практическое правило:

- при любых изменениях вокруг миграций, deploy и workday сначала убедитесь, что знаете фактический `DB_PATH` этого процесса, а уже потом делайте выводы о поведении БД.
