...
## Фаза 3 для вида «График»: зафиксированное состояние модели

Ниже факты о данных, от которых зависит корректный месячный график нагрузки:

1. Время задачи хранится агрегированно:
- источник истины: `tasks.timeSpentMs` (`timeSpent` в API);
- это итоговое время по задаче, не распределенное по датам;
- в модели нет встроенного day-by-day журнала времени.

2. Дата завершения задачи хранится на API:
- поле `tasks.completedAt` есть в SQLite и в API task shape;
- сервер заполняет `completedAt` при `done=true`, если дата не передана.

3. Фронтенд-модель после уточнения:
- фронтенд-нормализация (`apps/web/src/tasks-data.js`) сохраняет `completedAt` в runtime-узле задачи;
- `toggleTask/markTaskDone` поддерживают актуализацию `completedAt` при изменении статуса.

4. Workday-данные не заменяют помесячную модель:
- `workdays.summaryTimeMs/summaryDone` дают агрегаты по рабочему дню, но не полноценный исторический помесячный журнал по датам задач;
- `workdays.payload` — снимок состояния, а не нормализованный ежедневный таймлог.

Вывод для дальнейших фаз:
- Для `doneCount/day` минимальная предпосылка выполнена: `completedAt` доступен в API и фронтенд-модели.
- Для точного `spent/day` недостаточно только `timeSpentMs`; нужен отдельный механизм дневной фиксации времени (daily stats/worklog) или согласованная эвристика с ограничениями.
